...
impl TagService {
    ...
    ) -> Result<Vec<TagSelect>, ErrorResponse> {
        let mut filters = vec![];
    ...
    pub async fn get_owned_tag(
        &self,
        tag_id: String,
        owner_id: String,
    ) -> Result<Tag, ErrorResponse> {
        let tag = self
            .db
            .tag()
            .find_first(vec![
    ...
    pub async fn create_tag(
        ...
        tag_name: String,
        owner_id: String,
    ) -> Result<TagSelect, ErrorResponse> {
        let new_tag = self
            .db
            .tag()
            .create(tag_name, user::pk_user_id::equals(owner_id), vec![])
            .select(tag_select::select())
            .exec()
            .await?;
        Ok(new_tag)
    ...
    pub async fn delete_tag(&self, tag_id: String) -> Result<(), ErrorResponse> {
